...
def historia(request,template_name='historia.html'):    


    return render(request, template_name,{})


def conquistas(request,template_name='conquistas.html'):    
...
def noticias(request):
    ...
    # print(random_object)
    return render(request, 'front/noticias.html', {'site':site, 'news':news, 'cat':cat, 'subcat':subcat, 'lastnews':lastnews, 'popnews':popnews, 'popnews2':popnews2, 'trending':trending, 'lastnews2':lastnews2})


##--#--## Home Page(home) Function For Front (User Interface - Frontend) Start ##--#--##
def home(request):
   
    lastnews = News.objects.filter(act=1).order_by('-pk')[:6]   ### This query for last three post

    return render(request, 'front/home.html', {'lastnews':lastnews})
